...
it('renders a PDF for a room mixing a permanently blocked desk with real desks', function () {
    $teacher = User::factory()->create();
    $class = SchoolClass::factory()->for($teacher)->hasAttached(Subject::factory()->for($teacher))->create();
    $plan = SeatingPlan::factory()->for($teacher)->create();
    seatingDesk($plan, 0, 0, blocked: true);
    seatingDesk($plan, 0, 1);
...
it('renders a PDF even when every desk on the room layout is permanently blocked', function () {
    $teacher = User::factory()->create();
    $class = SchoolClass::factory()->for($teacher)->hasAttached(Subject::factory()->for($teacher))->create();
    $plan = SeatingPlan::factory()->for($teacher)->create();
    seatingDesk($plan, 0, 0, blocked: true);
