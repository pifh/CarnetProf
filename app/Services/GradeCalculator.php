...
class GradeCalculator
{
     ...
     * omit it for the annual average. Returns null if nothing is graded yet.
     */
    public function studentAverage(Student $student, SchoolClass $schoolClass, ?Term $term = null): ?float
    {
        $grades = Grade::query()
            ->where('student_id', $student->id)
            ->where('status', 'graded')
            ->whereNotNull('score')
            ->whereHas('evaluation', function ($query) use ($schoolClass, $term) {
     ...
     */
    public function classAverage(SchoolClass $schoolClass, ?Term $term = null): ?float
    {
        $averages = $schoolClass->students()
            ->where('is_archived', false)
            ->get()
            ->map(fn (Student $student) => $this->studentAverage($student, $schoolClass, $term))
